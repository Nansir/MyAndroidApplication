...
public class MP3Recorder {
		...
		new Thread() {
			...
			private void calculateRealVolume(short[] buffer, int readSize) {
				...
				if (readSize > 0) {
					...
					if (mVolume != volume && delegate != null
							&& delegate.get() != null) {
						delegate.get().onVolumeChangeListener(volume);
					}
					mVolume = volume;
				}
			};
	...
	private WeakReference<MP3RecorderDelegate> delegate;

	public void setDelegate(MP3RecorderDelegate d) {
		this.delegate = new WeakReference<MP3Recorder.MP3RecorderDelegate>(d);
	}

	public static interface MP3RecorderDelegate {
		/**
		 * 竟是变化回调 ， 子线程中执行,不要在里面执行耗时操作
		 * 
		 * @param volume
		 */
